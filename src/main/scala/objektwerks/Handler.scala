...
final class Handler(store: Store,
  ...
  val list = Map(
    ...
    Well -> listWells, Water -> listWaters, WaterHeater -> listWaterHeaters, Lawn -> listLawns
  )
  val add = Map(
    ...
    WaterHeater -> addWaterHeater, Lawn -> addLawn
  )
  val update = Map(
    ...
    Sewage -> updateSewage, Well -> updateWell, Water -> updateWater, WaterHeater -> updateWaterHeater
  )
