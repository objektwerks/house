...
final class Handler(store: Store,
                    emailer: Emailer):
  val list = Map(
    House -> listHouses, Drawing -> listDrawings, Foundation -> listFoundations, Frame -> listFrames, Attic -> listAttics,
  ...
  def listEntities(typeof: EntityType, parentId: Long): Event =
    Try {
      ...
      case NonFatal(error) => addFault( Fault(s"List entities [${typeof.toString}]{$parentId} failed: ${error.getMessage}") )
    }.get
