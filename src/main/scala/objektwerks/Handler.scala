...
final class Handler(store: Store,
  ...
  val list = Map(
    ...
    Electrical -> listElectricals, Fusebox -> listFuseboxes, Alarm -> listAlarms
  )
  val add = Map(
    ...
    Electrical -> addElectrical, Fusebox -> addFusebox, Alarm -> addAlarm
  )
  val update = Map(
    ...
    Door -> updateDoor, Plumbing -> updatePlumbing, Electrical -> updateElectrical, Fusebox -> updateFusebox
  )
