...
final class Handler(store: Store,
  ...
  val list = Map(
    ...
    Pool -> listPools, Dock -> listDocks, Gazebo -> listGazebos
  )
  val add = Map(
    ...
    SolarPanel -> addSolarPanel, Porch -> addPorch, Patio -> addPatio, Pool -> addPool, Dock -> addDock
  )
  val update = Map(
    ...
    Patio -> updatePatio, Pool -> updatePool, Dock -> updateDock
  )
