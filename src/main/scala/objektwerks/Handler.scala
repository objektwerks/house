...
final class Handler(store: Store,
  ...
  val list = Map(
    ...
    Sprinkler -> listSprinklers, Shed -> listSheds
  )
  val add = Map(
    ...
    WaterHeater -> addWaterHeater, Lawn -> addLawn, Garden -> addGarden, Sprinkler -> addSprinkler
  )
  val update = Map(
    ...
    Garden -> updateGarden, Sprinkler -> updateSprinkler
  )
