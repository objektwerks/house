...
final class Store(cache: Cache[String, String],
  ...
  def addHouse(house: House): Long =
    DB localTx { implicit session =>
      sql"""
        insert into house(account_id, typeof, location, label, note, architect, builder, built)
        values(${house.accountId}, ${house.typeof.toString}, ${house.location}, ${house.label}, ${house.note},
        ${house.architect}, ${house.builder}, ${house.built})
        """
        .updateAndReturnGeneratedKey()
    }
  ...
  def addWaterHeater(waterHeater: WaterHeater): Long =
    DB localTx { implicit session =>
      sql"""
        insert into water_heater(house_id, typeof, label, note, installed)
        values(${waterHeater.houseId}, ${waterHeater.typeof.toString}, ${waterHeater.label}, ${waterHeater.note}, ${waterHeater.installed})
        """
        .updateAndReturnGeneratedKey()
    }
  ...
  def updateWaterHeater(waterHeater: WaterHeater): Int =
    DB localTx { implicit session =>
      sql"""
        update water_heater set typeof = ${waterHeater.typeof.toString}, label = ${waterHeater.label}, note = ${waterHeater.note},
        installed = ${waterHeater.installed} where id = ${waterHeater.id}
        """
        .update()
    }
  ...
  def addGarden(garden: Garden): Long =
    DB localTx { implicit session =>
      sql"""
        insert into garden(house_id, typeof, label, note, planted)
        values(${garden.houseId}, ${garden.typeof.toString}, ${garden.label}, ${garden.note}, ${garden.planted})
        """
        .updateAndReturnGeneratedKey()
    }
  ...
  def addSprinkler(sprinkler: Sprinkler): Long =
    DB localTx { implicit session =>
      sql"""
        insert into sprinkler(house_id, typeof, label, note, installed)
        values(${sprinkler.houseId}, ${sprinkler.typeof.toString}, ${sprinkler.label}, ${sprinkler.note}, ${sprinkler.installed})
        """
        .updateAndReturnGeneratedKey()
    }
  ...
  def updateShed(shed: Shed): Int =
    DB localTx { implicit session =>
      sql"""
        update shed set typeof = ${shed.typeof.toString}, label = ${shed.label}, note = ${shed.note}, built = ${shed.built}
        where id = ${shed.id}
        """
        .update()
    }
  ...
  def addPool(pool: Pool): Long =
    DB localTx { implicit session =>
      sql"""
        insert into pool(house_id, typeof, gallons, caged, label, note, built)
        values(${pool.houseId}, ${pool.typeof.toString}, ${pool.gallons}, ${pool.caged}, ${pool.label}, ${pool.note}, ${pool.built})
        """
        .updateAndReturnGeneratedKey()
    }
  ...
  def addDock(dock: Dock): Long =
    DB localTx { implicit session =>
      sql"""
        insert into dock(house_id, typeof, label, note, built)
        values(${dock.houseId}, ${dock.typeof.toString}, ${dock.label}, ${dock.note}, ${dock.built})
        """
        .updateAndReturnGeneratedKey()
    }
