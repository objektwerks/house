...
final class Handler(store: Store,
  ...
  val list = Map(
    ...
    AirConditioner -> listAirConditioners, Floor -> listFloors, Lighting -> listLightings
  )
  val add = Map(
    ...
    Floor -> addFloor, Lighting -> addLighting
  )
  val update = Map(
    ...
    Heater -> updateHeater, AirConditioner -> updateAirConditioner, Floor -> updateFloor
  )
