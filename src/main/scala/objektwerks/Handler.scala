...
final class Handler(store: Store,
  ...
  val list = Map(
    ...
    Siding -> listSidings, Gutter -> listGutters, Soffit -> listSoffits, Window -> listWindows
  )
  val add = Map(
    ...
    Gutter -> addGutter, Soffit -> addSoffit, Window -> addWindow
  )
  val update = Map(
    ...
    Garage -> updateGarage, Siding -> updateSiding, Gutter -> updateGutter, Soffit -> updateSoffit
  )
